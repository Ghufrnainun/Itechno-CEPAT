  ...
  const handleSaveEdit = async () => {
    ...
    try {
      if (hasApiData) {
        ...
        const data = await res.json().catch(() => ({}));
        if (!res.ok || !data.success) {
          throw new Error(data.message || "Gagal menyimpan perubahan ke server");
        }
      }
              ...
              {editSkills.map((skill, idx) => (
                <div key={idx} className="flex flex-col gap-2.5 p-3.5 border border-card-border rounded-xl bg-surface-container-low relative">
                  <button
                    ...
                    className="absolute top-2.5 right-2.5 text-on-surface-variant hover:text-error transition-colors w-7 h-7 flex items-center justify-center rounded-lg hover:bg-error-container/30"
